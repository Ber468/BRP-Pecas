...
const ProdutoList = (props) => {
  const countryBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <button
          onClick={() => props.editar(rowData.id_produto)}
          className="btn btn-warning"
        >
          Editar
        </button>
        <button
          onClick={() => props.excluir(rowData.id_produto)}
  ...
  return (
    <div>
      <div>
        <h4 style={{marginTop:"2%"}}>Listagem de Produtos</h4>
        <button
        ...
        <button style={{margin: '8px'}} type="button" className="btn btn-light btn-sm">
          <Link to="/tipoProduto" style={{textDecoration: "none", color: "black"}}>Tipo Produto</Link>
        </button>
        <button 
        style={{margin: '8px'}} 
        type="button" 
