    ...
    'CurrentPageReport': (options) => {
        ...
        const ClienteList = (props) => {
            const countryBodyTemplate = (rowData) => {
                return (
                    <React.Fragment>
                        <button onClick={() => props.editar(rowData.id_cliente)} className="btn btn-warning">Editar</button>
                        <button onClick={() => props.excluir(rowData.id_cliente)} className="btn btn-danger">Excluir</button>
                    </React.Fragment>
                );
            }
            return (
                        ...
                        <div className="card">
                            <DataTable value={props.clientes} responsiveLayout="scroll" selectionMode="single" paginator paginatorTemplate={template2} rows={5}
                                paginatorClassName="justify-content-center" className="mt-6">
                                    ...
                                    <Column field="endereco" header="Endereço" sortable filter></Column>
