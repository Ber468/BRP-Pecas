...
function ClienteCont() {
    ...
    const editar = (id_cliente) => {
        setCliente(clientes.filter((cliente) => cliente.id_cliente === id_cliente)[0]);
        setEditando(true);
    };

    const excluir = (id_cliente) => {
        confirmDialog({
            ...
            accept: () => excluirConfirm(id_cliente),
        });
    };

    const excluirConfirm = (id_cliente) => {
        ClienteSrv.excluir(id_cliente)
            .then((response) => {
                onClickAtualizar();
                toastRef.current.show({
                    severity: "success",
