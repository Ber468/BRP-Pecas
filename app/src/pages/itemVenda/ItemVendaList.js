...
const ItemVendaList = (props) => {
  const countryBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <button
          onClick={() => props.editar(rowData.id_itemvenda)}
          className="btn btn-warning"
        >
          Editar
        </button>
        <button
          onClick={() => props.excluir(rowData.id_itemvenda)}
  ...
  return (
    <div>
      <div>
        <h4 style={{marginTop:"2%"}}>Listagem de Itens Vendas</h4>
        <button
        ...
        <button style={{margin: '8px'}} type="button" className="btn btn-light btn-sm">
          <Link to="/venda" style={{textDecoration: "none", color: "black"}}>Voltar</Link>
        </button>
        <div className="card">
          <DataTable
