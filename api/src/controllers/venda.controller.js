...
exports.listAllVenda = async (req, res) => {
  const response = await db.query('SELECT venda.id_venda, venda.data, venda.valorTotal, usuario.nome as usuario, ' 
...
exports.findVendaById = async (req, res) => {
  ...
  const response = await db.query('SELECT venda.data, venda.valorTotal, usuario.nome as usuario, cliente.nome as cliente from venda '
+'inner join usuario on venda.id_venda = usuario.id_usuario inner join cliente on venda.id_venda = cliente.id_cliente '
+'WHERE id_venda = $1', [id_venda]);
  res.status(200).send(response.rows);
}
