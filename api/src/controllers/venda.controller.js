...
exports.createVenda = async (req, res) => {
  const { data, id_usuario, id_cliente } = req.body;
  ...
  const { rows } = await db.query(
    "INSERT INTO venda (data, id_usuario, id_cliente) VALUES ($1, $2, $3)",
    [data, id_usuario, id_cliente]
  );
  console.log(rows);
  res.status(201).send({
    message: "Venda adicionado com sucesso!",
    body: {
      venda: { data, id_usuario, id_cliente },
    },
  });
};
};
...
exports.updateVendaById = async (req, res) => {
  ...
  const response = await db.query(
    "UPDATE venda SET data = $1, id_usuario = $2, id_cliente = $3 WHERE id_venda = $4",
    [data, id_usuario, id_cliente, id_venda]
  );
