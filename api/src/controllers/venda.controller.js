...
exports.createVenda = async (req, res) => {
  const { descricao, data, valorTotal, id_usuario, id_cliente } = req.body;
  const { rows } = await db.query(
    "INSERT INTO venda (descricao, data, valorTotal, id_usuario, id_cliente) VALUES ($1, $2, $3, $4, $5)",
    [descricao, data, valorTotal, id_usuario, id_cliente ]
  );

  res.status(201).send({
    message: "Venda adicionado com sucesso!",
    body: {
      venda: { descricao, data, valorTotal, id_usuario, id_cliente }
    },
  });
};
...
exports.updateVendaById = async (req, res) => {
  ...
  const response = await db.query(
    "UPDATE venda SET descricao = $1, data = $2, valorTotal = $3, id_usuario = $4, id_cliente = $5 WHERE id_venda = $6",
    [descricao, data, valorTotal, id_usuario, id_cliente]
  );
