...
exports.listAllItemPedido = async (req, res) => {
  const response = await db.query('SELECT itemPedido.id_itemPedido, itemPedido.quantidade, pedido.descricao as pedido, produto.nome as produto ' 
...
exports.findItemPedidoById = async (req, res) => {
  ...
  const response = await db.query('SELECT itemPedido.quantidade, pedido.descricao as pedido, produto.nome as produto '  
+'from itemPedido inner join pedido on itemPedido.id_pedido = pedido.id_pedido '
+'inner join produto on itemPedido.id_produto = produto.id_produto WHERE id_itemPedido = $1', [id_itemPedido]);
  res.status(200).send(response.rows);
}
...
exports.deleteItemPedidoById = async (req, res) => {
  ...
  res.status(200).send({ message: 'ItemPedido deletada com sucesso!', id_itemPedido });
};
